...
#default connection properites DEMO: main port: 5124, streaming port: 5125,
...
class JsonSocket(object):
    ...
    def _read(self, bytesSize=4096):
        ...
        globmessages.append('Received: ' + str(resp))
        return resp
...
class APIStreamClient(JsonSocket):
    def __init__(self, address=DEFAULT_XAPI_ADDRESS, port=DEFUALT_XAPI_STREAMING_PORT, encrypt=True, ssId=None, 
                 tickFun=None, tradeFun=None, balanceFun=None, tradeStatusFun=None, profitFun=None, newsFun=None, message=[],):
...
def main():
    ...
    # enter your login credentials here NOT LOGIN , THE ACCOUNT NUMBER 
    userId = 13755569
    ...
    # second method of invoking commands : getAllSymbols--2 min 10 secound
    #resp = client.commandExecute('getAllSymbols')
    #resp = client.commandExecute('getSymbol',{"symbol": "EURUSD"})

    s = "01/09/2022"
    rtime = int(time.mktime(datetime.datetime.strptime(s, "%d/%m/%Y").timetuple())*1000)
    #print(rtime)
    ...
    #resp = client.commandExecute('tradeTransaction',{"tradeTransInfo": TRADE_TRANS_INFO})


    #print('Enter your order number:')
    #order = int(input())
    #resp = client.commandExecute('tradeTransactionStatus',{"order": order})

    #resp = client.commandExecute('getTradesHistory',{"end": 0,"start": 0})  #history for closed trades

   # resp = client.commandExecute('getTrades',{"openedOnly": True})
    #resp = client.commandExecute('getTrades',{"openedOnly": False})
    ...
    c= log_stream.getvalue()
    print("aaaaaaaaaaaaaaaaaaaa")
    for m in globmessages:
        print(m)
